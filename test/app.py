...
app.config['MYSQL_PASSWORD'] ='Saron123.'
app.config['MYSQL_DB'] = 'user_management'
print("PASSWORD BEING USED:", app.config.get('MYSQL_PASSWORD'))
print("ENV PASSWORD:", os.getenv('MYSQL_PASSWORD'))
...
def sync_tasks_for_job_order(cur, job_order_id):
    """Keep task assignees aligned with a job order and create a default task when needed."""
    cur.execute(
        """
        SELECT job_order_id,
    ...
    _, assigned_employee_id, title, description, status = job_row
    ...
    task_count = task_count_row[0] if task_count_row else 0
    if task_count:
        return

    default_task_name = (title or '').strip() or f'Job Order #{job_order_id}'
    default_status = (status or '').strip() or 'Scheduled'
    ...
    cur.execute(
        ...
        (
            ...
            description,
            default_status,
            datetime.now() if default_status.lower() == 'completed' else None
        )
    )


def sync_tasks_for_employee_assignments(cur, employee_id):
    """Backfill missing tasks for assigned jobs and repair stale task assignees."""
    cur.execute(
        """
        SELECT job_order_id
        FROM job_orders
    ...
    for (job_order_id,) in job_rows:
        sync_tasks_for_job_order(cur, job_order_id)
...
def update_employee(user_id):
    ...
    try:
        cur = mysql.connection.cursor()
        if 'first_name' in data or 'last_name' in data or 'phone' in data or 'password' in data:
            user_fields = {}
            ...
            if 'password' in data:
                ...
                user_fields['password'] = generate_password_hash(raw_password if raw_password.strip() else 'test123')
            if user_fields:
...
def task_management_dashboard_page():
    ...
    try:
        cur = mysql.connection.cursor()
        employee_id = ensure_employee_record(cur, current_user.id, 'Staff')
        sync_tasks_for_employee_assignments(cur, employee_id)
        cur.execute("""
            SELECT jo.job_order_id,
                   jo.title,
                   jo.description,
                   ...
                   CONCAT(a.street_1, ', ', a.city, ', ', a.state, ' ', a.zip_code) AS location
            FROM job_orders jo
            ...
            WHERE jo.assigned_employee_id = %s
            ORDER BY jo.scheduled_date DESC, jo.job_order_id DESC
        """, (employee_id,))
        rows = cur.fetchall()
        for r in rows:
            job_order_id, job_title, job_description, job_status, sched_date, start_t, end_t, company, contact_name, contact_phone, contact_email, location = r
            cur.execute("""
                SELECT task_id, task_name, description, status
                FROM tasks
                WHERE job_order_id = %s
                  AND (assigned_employee_id = %s OR assigned_employee_id IS NULL)
                ORDER BY task_id
                LIMIT 1
            """, (job_order_id, employee_id))
...
def add_user():
    if request.is_json:
        ...
        raw_password = str(raw_password)
        password = generate_password_hash(raw_password.strip() or 'test123')
        phone = data.get('phone')

        cur = mysql.connection.cursor()

        cur.execute("SELECT user_id FROM users WHERE LOWER(TRIM(email)) = %s LIMIT 1", (email,))
        if cur.fetchone():
            cur.close()
    ...
    try:
        ...
        for day_str, day_data in availability_data.items():
            try:
                ...
                if available_from == 'Unavailable' or available_to == 'Unavailable':
                    is_available = False
                    available_from = None
                    available_to = None

                # Treat partial time windows as unavailable so mixed values don't persist.
...
def get_employee_availability(employee_id):
    ...
    try:
        ...
        if not is_management:
            current_emp_id = ensure_employee_record(cur, current_user.id, 'Staff')
            is_own_record = bool(current_emp_id == resolved_employee_id)
...
def staff_schedule_events():
    ...
    try:
        ...
        events = []
        for row in rows:
            job_order_id, title, sched_date, start_t, end_t, status = row
            if not sched_date:
                continue
